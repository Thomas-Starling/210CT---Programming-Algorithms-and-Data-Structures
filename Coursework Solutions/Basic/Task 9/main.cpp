...
/*
    ...
    see if their are any values between the users input, same structure as the regular binary search algorithm
    however it sees if the middle value in the sequence is between the users input, if there are value/values
    then return true otherwise it will increment or decrement the start or end depending on whether the value
    is higher or lower.

    The expected output if there is a value between high and low is true, which is then passed to the main
    function then assessed to see if it is true or false, if the value is true then the output is true
    ...
    algorithm. The algorithm complexity of binary search is O(log n) this is because you are dividing the
...
{
    //Iterative approach
    int start = 0;  //Beginning of the sequence
...
/*
    The main function creates all the variables and generates the sequence, the sequence is randomly generated
    then sorted using bubblesort becuase binary search only searchs a sorted sequence. The user is presented
    with the sorted sequence for them to input numbers high and low, these are the boundries for the algorithm
    to search between.

    The function is called within a variable and the result is stored inside it, after the output contains a
    tennery statement where it is compared to see if the returned value is true or false.
*/
